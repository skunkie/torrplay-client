...
export default function HomePage() {
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await deleteTorrent(torrentToDelete.hash);
      toast.success('Torrent deleted', {
        description: `Successfully deleted ${torrentToDelete.title || torrentToDelete.hash}`,
      });
      mutate();
      closeDeleteDialog();
    } catch (error) {
  ...
  const handlePlay = async (torrent: Torrent) => {
    ...
    try {
      const fullTorrentDetails = await getTorrent(torrent.hash);
          ...
          <div className='py-4'>
            {torrentToDelete && (
              <p className='text-center font-medium break-words'>
                &quot;{torrentToDelete.title || torrentToDelete.name || torrentToDelete.hash}&quot;
              </p>
            )}
          </div>
